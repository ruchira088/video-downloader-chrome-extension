...
class VideoDownloaderApiImpl implements VideoDownloaderApi {
  ...
  videoExistsByUrl(videoUrl: string): Promise<boolean> {
    return fetch(
      `${this.apiConfiguration.url}/schedule/search?video-url=${videoUrl}`,
      {
        headers: {
          Authorization: `Bearer ${this.apiConfiguration.authenticationToken}`,
        },
      ...
      .then((response) =>
        response
          .json()
          .then((body) =>
            response.ok ? Promise.resolve(body) : Promise.reject(body)
          )
      )
...
const apiConfiguration = (
  ...
  Promise.all([
    keyValueStore.get(StorageKey.ApiServerUrl),
    keyValueStore.get(StorageKey.AuthenticationCookie),
...
export default () =>
  apiConfiguration(storageAreaKeyValueStore()).then(
    (config) => new VideoDownloaderApiImpl(config)
  );
