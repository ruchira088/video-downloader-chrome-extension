...
window.onload = () => {
  ...
  const apiServerTextInput = Maybe.fromFalsy(document.getElementById("api-server-input")) as Maybe<HTMLInputElement>

  const keyValueStore = storageAreaKeyValueStore()

  saveButton.forEach((button) => {
    apiServerTextInput.forEach((textInput) => {
      showApiServerUrl(keyValueStore, textInput).then(() => addButtonClickHandler(keyValueStore, button, textInput))
    })
  })
}
